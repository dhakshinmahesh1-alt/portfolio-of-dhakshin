...
import { Gear, Wrench, Lightbulb } from './Illustrations'

const skills = [
  {
    category: 'Vibecoding',
    items: ['Claude Code', 'Codex', 'OpenCode', 'AI-Assisted Coding'],
  },
  {
    category: 'Hardware',
  ...
  {
    category: 'Platforms',
    items: ['Vercel', 'GitHub', 'Firebase', 'Supabase'],
  },
  {
    category: 'AI / APIs',
    items: ['OpenAI API', 'Claude API', 'REST APIs', 'Prompt Engineering'],
  },
  {
    category: 'Interests',
    items: ['RC Cars', 'LED Projects', 'Drones', 'IoT'],
  },
  {
    category: 'Life',
    ...
    <section id="skills" className="py-32 px-6 bg-white relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <motion.div
          ...
          viewport={{ once: true }}
          className="absolute top-12 right-[8%] rotate-30"
        >
          <Wrench size={80} />
        </motion.div>
        <motion.div
          ...
          viewport={{ once: true }}
          className="absolute bottom-20 left-[5%] -rotate-15"
        >
          <Lightbulb size={60} />
        </motion.div>
      </div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {s.items.map((item) => (
                  <span
                    key={item}
                    className="text-xs px-2.5 py-1 rounded-full bg-white text-text-secondary border border-border/50"
                  >
                    {item}
                  </span>
                ))}
